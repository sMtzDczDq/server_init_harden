...
function revert_source_list_changes() {
	...
	if [[ ${#SOURCE_FILES_BKP[@]} -gt 0 ]]; then
		for file in "${SOURCE_FILES_BKP[@]}"; do
...
{
	if [[ $AUTO_GEN_USERNAME == 'y' ]]; then
		NORM_USER_NAME="$(tr < /dev/urandom -cd 'a-z' | head -c 6)""$(tr < /dev/urandom -cd '0-9' | head -c 2)" || exit 1
		file_log "Generated user name ${NORM_USER_NAME}"
	fi

	# Generate a 15 character random password
	USER_PASS="$(tr < /dev/urandom -cd "[:alnum:]" | head -c 15)" || exit 1
...
setup_step_start "${STEP_TEXT[1]}"
{
	...
	KEY_PASS="$(tr < /dev/urandom -cd "[:alnum:]" | head -c 15)"
...
if [[ $DEFAULT_SOURCE_LIST == "y" ]]; then
	...
	setup_step_start "${STEP_TEXT[4]}"
	{
		...
		if [[ $CODE_NAME == "bookworm" ]]; then
			cat << BOOKWORM >> /etc/apt/sources.list
...
if [[ $RESET_ROOT_PWD == 'y' ]]; then
	setup_step_start "${STEP_TEXT[8]}"
	{
		...
		PASS_ROOT="$(tr < /dev/urandom -cd "[:alnum:]" | head -c 15)"
		set_exit_code $?
...
function set_config_key() {
	...
	if [[ $(grep -Pnc "$INACTIVE_KEYS_REGEX" "$file_location") -eq 0 ]] && [[ $(grep -Pnc "$ACTIVE_KEYS_REGEX" "$file_location") -eq 0 ]]; then
	...
	if [[ $(grep -Pnc "$ACTIVE_KEYS_REGEX" "$file_location") -gt 0 ]]; then
		...
		grep -Pn "$ACTIVE_KEYS_REGEX" "$file_location" | while read -r i; do
			# Get the line number
	...
	if [[ $(grep -Pnc "$INACTIVE_KEYS_REGEX" "$file_location") -gt 0 ]] && [[ $(grep -Pnc "$ACTIVE_KEYS_REGEX" "$file_location") -eq 0 ]]; then
